...
const BANK_MASTER_METADATA_CACHE_KEY = 'bank-master-metadata-v1';
...
function buildBankMasterMetadata(summary: BankMasterData[]): BankMasterMetadata {
  const activeCount = summary.reduce((count, bank) => (bank.isActive ? count + 1 : count), 0);

  const uniqueStates = Array.from(
    new Set(
      summary.map((bank) => bank.state?.trim()).filter((state): state is string => Boolean(state))
    )
