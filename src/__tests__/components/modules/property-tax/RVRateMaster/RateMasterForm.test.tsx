  it("shows correct configured rates count in delete confirmation", async () => {
    // Mock backend rates: 2 with non-zero, 1 with zero
    const backendRates = [
      {
        Id: 1,
        id: 1,
    ...
    // Patch the confirm dialog to capture the description
    let confirmDescription = "";
    vi.doMock("@/components/common/ConfirmProvider", () => ({
      useConfirm: () => ({
        confirm: vi.fn(({ description, onConfirm }) => {
          confirmDescription = description;
          if (onConfirm) onConfirm();
        }),
      }),
    }));
    // Patch useRateMasterFormState to return only known properties
    vi.mocked(useRateMasterFormState).mockReturnValue({
      ...
      filledRatesCount: 0,
      totalPossibleRates: 0,
      completionPercentage: 0,
      matrixStorageKey: "test-key",
      handleMatrixPaginationChange: vi.fn(),
      buildCompleteMatrixForSubmission: vi.fn(),
    });
    // Render in delete mode, pass backendRates as prop
    const deleteProps = { ...mockProps, mode: "delete" as const, id: "1", backendRates };
    render(
      <NextIntlClientProvider locale="en" messages={mockMessages}>
        <RateMasterForm {...deleteProps} />
      </NextIntlClientProvider>
    );
    // Simulate delete action (implementation may vary)
    // The confirm dialog should show count: 2 (only non-zero rates)
    await waitFor(() => {
      expect(confirmDescription).toMatch(/2/);
    });
  });
