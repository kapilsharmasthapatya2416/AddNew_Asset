...
function useLoginT() {
    ...
    return useCallback((key: string, values?: Record<string, string | number | Date>) => {
        return (tc as (fullKey: string, v?: Record<string, string | number | Date>) => string)(`login.${key}`, values);
    }, [tc]);
}
...
function LoginCredentialFields({
    ...
    return (
        <>
            ...
            <motion.div whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} className="flex justify-center pt-4">
        ...
        >
            <Card className="overflow-hidden rounded-2xl border border-white/40 bg-white/80 shadow-2xl backdrop-blur-md transition-all duration-500 ease-in-out">
                <CardHeader className="flex flex-col items-center space-y-1 pb-2 pt-8 text-center">
                    ...
                    <div className="pt-1 text-sm font-bold uppercase tracking-[0.2em] text-cyan-600">{t('title')}</div>
                </CardHeader>
