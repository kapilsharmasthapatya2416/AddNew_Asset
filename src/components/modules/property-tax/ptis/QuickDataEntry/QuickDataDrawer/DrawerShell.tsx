...
export default function DrawerShell({ children, locale }: DrawerShellProps) {
    ...
    const propertyId = routePropertyId || searchParams.get('propertyId') || '';
    ...
    const handleClose = () => {
        const closeQuery = new URLSearchParams();
        if (wardNo) closeQuery.set('wardNo', wardNo);
        if (propertyNo) closeQuery.set('propertyNo', propertyNo);
        if (partitionNo) closeQuery.set('partitionNo', partitionNo);
        if (propertyId) closeQuery.set('propertyId', propertyId);

        const finalQuery = closeQuery.toString();
        router.push(`/${locale}/property-tax/ptis${finalQuery ? `?${finalQuery}` : ''}`);
    };
                            ...
                            <nav className="flex items-center gap-2 min-w-max md:grid md:grid-cols-7">
                                {TABS.map((tab) => {
                                    const isActive = pathname
                                        .split('/')
                                        .some(segment => segment.toLowerCase() === tab.href.toLowerCase());
                                    ...
                                    return (
                                        <Link
                                            key={tab.href}
                                            href={`/${locale}/property-tax/ptis/QuickDataEntry/${propertyId}/${tab.href}${queryString ? `?${queryString}` : ''}`}
                                            className={cn(
