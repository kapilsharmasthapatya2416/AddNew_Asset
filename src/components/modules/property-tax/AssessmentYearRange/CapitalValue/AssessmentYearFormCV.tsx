...
import { createAssessmentYearActionCV, updateAssessmentYearActionCV } from "@/app/[locale]/property-tax/assessment-year-range/capitalvalue/action";
...
export default function AssessmentYearFormCV({ open, onClose, onSuccess, initialData }: AssessmentYearFormCVProps) {
  ...
  const [formData, setFormData] = useState<Partial<AssessmentYearCV>>(initial);
      ...
      footer={
        <>
          ...
          <SaveButton
            ...
            type="submit"
            form="assessment-year-form-cv"
            isLoading={isSubmitting}
          />
        </>
      }
    >
      <form id="assessment-year-form-cv" onSubmit={handleSubmit} className="flex flex-col gap-6 h-full">
         {/* Form Fields */}
                ...
                <div className="rounded-xl border border-[#DCEAFF] bg-white p-4 mb-4">
                  ...
                  >
                    ...
                    <ToggleSwitch
                      ...
                      onChange={() => setFormData((prev: Partial<AssessmentYearCV>) => ({ ...prev, isActive: !prev.isActive }))}
                      showPopup={false}
                    />
